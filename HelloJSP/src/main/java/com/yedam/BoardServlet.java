...
// 페이지 : 75
// live server : 
// client -> 웹서버(정적페이지) -> 서블릿 컨테이너(톰캣) -> init() -> service() -> destroy()
...
public class BoardServlet extends HttpServlet
{
	...
	@Override
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException    // 매번 실행
	{
		if(req.getMethod().equals("GET"))   //조회
		{
			...
			String board_no = req.getParameter("board_no"); // "3"   // board_no=3&writer=user01 -> parameter
			...
			html += "<tr><th>글번호</th><td>" + board.getBoardNo() + "</td><th>제목</th><td>" + board.getTitle() + "</td></tr>";
