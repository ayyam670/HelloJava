...
public class BoardListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,// 요청정보
		...
		//response.getWriter().append("Served at: ").append(request.getContextPath());  교수님은 그냥 지우셨음
		...
		List<BoardVO> list = svc.boardList(new SearchDTO());
		
		String html = "<table border='2'><thead><tr><th>글번호</th><th>제목</th><th>작성자></th><th>조회수</th></tr></thead>";
		html += "<tbody>";
		for(BoardVO board : list)
		{
			html += "<tr><td align='center'><a href = 'board?board_no="+board.getBoardNo()+"'>" + board.getBoardNo() + "</td><td> " + board.getTitle()
